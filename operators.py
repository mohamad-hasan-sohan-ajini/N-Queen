...
def recombination(permutation1, permutation2):
    ...
    for i in range(cross_over_point, n):
        # 'j' is parent index
        for j in range(n):
            if not(permutation2[(j+cross_over_point)%n] in child1) and (child1[i] == -1):
                child1[i] = permutation2[(j+cross_over_point)%n]
            if not(permutation1[(j+cross_over_point)%n] in child2) and (child2[i] == -1):
...
def generate_permutation(n):
    return list(list(itertools.permutations(range(1, n+1)))\
